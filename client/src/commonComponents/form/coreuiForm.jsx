...
const CoreUIForm = (props) => {
  const { render, model, className, onSubmit, onUpdateData, ...nestedProps } = props
  ...
  return (<><CForm action='' method='post' className={className || 'mb-3'} {...nestedProps}>
    {Object.keys(model).filter((key1) => (model[key1].isForm)).map((key2, index) => {
      ...
      return <CFormGroup key={`field-${key2}-${index}`} style={{
        ...
        {component ? component({ ...nestedProperties, onChange: onChangeField })
          : <TextWithLabel
